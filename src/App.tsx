...
export default function App() {
  ...
  useEffect(() => {
    const checkUpstox = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/upstox/connection-info`);
    ...
    const fetchMD = async () => {
      try {
        const [sectors, sentiment, momentum, indices] = await Promise.all([
          fetch(`${API_BASE}/api/quant/sectors`).then(r => r.json()),
          fetch(`${API_BASE}/api/quant/sentiment`).then(r => r.json()),
          fetch(`${API_BASE}/api/quant/momentum`).then(r => r.json()),
          fetch(`${API_BASE}/api/market/indices`).then(r => r.json()),
        ]);
  ...
  useEffect(() => {
    ...
    const sse = new EventSource(
      `${API_BASE}/api/stocks/stream?instrumentKey=${encodeURIComponent(selectedStock.key)}&symbol=${encodeURIComponent(selectedStock.symbol)}`
    );
    sseRef.current = sse;
